...
import { useCreateAdminMutation } from "@/api-integration/mutations/admin";

export default function AdminTable() {
  const navigate = useNavigate();
  const { data: users = [], isLoading, isError } = useUsersQuery("admin");
      ...
      <Dialog open={showInvite} onOpenChange={setShowInvite}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Enter the admin&rsquo;s full name and email. A temporary password will be generated.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                createAdmin.mutate(
                  { email: inviteEmail, name: inviteName, password: tempPassword },
                  {
                    onSuccess: (res) => {
                      setShowCreds({ open: true, name: inviteName, email: res.email, password: tempPassword });
                      setInviteEmail("");
                      setInviteName("");
                      setShowInvite(false);
                    ...
                    onError: (err: unknown) => {
                      ...
                      toast.error(msg || "Failed to create admin user");
                    },
                  }
                );
              }}
              disabled={createAdmin.isPending || !inviteEmail || !inviteName}
            >
