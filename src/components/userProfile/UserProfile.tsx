...
export default function UserProfile({className, textSize}: {className?: string, textSize?: string}) {
  ...
  return (
      <section className={`${className} items-center justify-between w-full max-h-[5rem] dark:border-gray-600 pr-1`}>
        <div className="w-auto h-auto flex items-center justify-center bg-gray-500/40 dark:bg-transparent rounded-full shadow-lg border-2 border-[#56bbe3] p-1">
          ...
          <div className={`text-left flex flex-col ${textSize || 'text-xs md:text-sm'} text-gray-400 dark:text-[#56bbe3] ml-1`}>
            <b className="">{displayName}</b>
            <p className="text-gray-500">{roleLabel}</p>
        </div>
        <span className="w-[0.2rem] h-[2.1rem] bg-gray-500 dark:bg-gray-500 rounded-full text-gray-100 dark:text-gray-500 mx-1"></span>
        <Link to="/settings" className="flex items-center dark:text-[#56bbe3] text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-all duration-300 ease-in-out">
            <Settings />
            <EllipsisVertical />
        </Link>  
    </section>
