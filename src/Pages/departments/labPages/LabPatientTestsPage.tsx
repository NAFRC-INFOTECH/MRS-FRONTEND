...
export default function LabPatientTestsPage() {
  ...
  const patientReferral = useMemo(() => {
    const matches = (referrals as LabReferral[]).filter(
      (ref) => String(ref.patientId) === String(patientId)
    );
    return matches[0];
  }, [referrals, patientId]);
  ...
  const validateForm = () => {
    ...
    requiredTests.forEach((test) => {
      if (!results[test]?.trim()) {
        newErrors[test] = "This field is required";
      }
    });
  ...
  const onSave = () => {
    ...
    if (!isValid) {
      toast.error("Please complete all required test results");
      return;
    }
    ...
    updateResults.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Patient test results saved");
        },
        onError: () => {
          toast.error("Failed to save patient test results");
        },
      }
    );
  };
  ...
  return (
    <div className="space-y-6 py-4">
      <div className="flex items-center justify-between gap-20">
        <div>
          <h2 className="text-xl font-semibold">Patient Required Tests</h2>
          <p className="text-sm text-gray-500">
            Required tests and result input for the selected patient.
          </p>
        </div>

        <Button variant="outline" onClick={() => navigate("/lab/patient-list")}>
          Back
        </Button>
      </div>
      ...
      {!isLoading && !patientReferral && (
        <div className="rounded-lg border border-gray-200 bg-white p-4 text-sm text-gray-500">
          No lab referral found for this patient.
        </div>
      )}
              ...
              <tbody>
                    ...
                    <td className="px-4 py-3 min-w-[18rem]">
                      <div className="space-y-1">
                        <Input
                          ...
                          value={results[test] || ""}
                          onChange={(e) =>
                            onChangeResult(test, e.target.value)
                          }
                ...
                {requiredTests.length === 0 && (
                  <tr>
                    ...
                    >
                      No required tests listed for this patient.
                    </td>
                  </tr>
                )}
              </tbody>
