...
export default function RecordingDailyShift() {
  ...
  const [role, setRole] = useState<"recording">("recording");
  ...
  const mappedRecordingUsers = useMemo(
    () => recordingUsers.map((d: any) => ({ id: d._id, name: d.name })),
    [recordingUsers]
  );
  ...
  const exportCsv = () => {
    const headers = ["Recording Staff", "Department", "Date", "Shift", "Time In", "Time Out", "Status"];
    const rows = duties.map((d) => {
      const staffName = mappedRecordingUsers.find((x) => x.id === (d as any).recordingUserId)?.name || "-";
      ...
      return [staffName, deptName, dateText, shiftText, timeInText, timeOutText, statusText];
    });
    ...
    a.href = url;
    a.download = `recording_duties_${dateFilter || "today"}.csv`;
    ...
    <div className="py-4 space-y-6">
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Popover open={openCombobox} onOpenChange={setOpenCombobox}>
              <PopoverTrigger asChild>
                <Button variant="outline" type="button" className="justify-between">
                  {mappedRecordingUsers.find((n) => n.id === staffId)?.name || "Select Staff"}
                </Button>
              </PopoverTrigger>
          ...
          <div className="flex flex-col gap-1">
            <Label htmlFor="recording-duty-date">Duty Date</Label>
            <Input id="recording-duty-date" type="date" value={date} onChange={(e) => { const v = e.target.value; setDate(v); applyShiftTimes(v, shift); }} />
          </div>
          <div className="flex flex-col gap-1">
            <Label>Shift</Label>
          ...
          <div className="flex flex-col gap-1">
            <Label htmlFor="recording-time-in">Time In</Label>
            <Input id="recording-time-in" type="datetime-local" value={timeIn} onChange={(e) => setTimeIn(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1">
            <Label htmlFor="recording-time-out">Time Out</Label>
            <Input id="recording-time-out" type="datetime-local" value={timeOut} onChange={(e) => setTimeOut(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1">
            <Label>Status</Label>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="flex flex-col gap-1">
              <Label htmlFor="recording-filter-date">Filter Date</Label>
              <Input id="recording-filter-date" type="date" value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} />
            </div>
            <div className="flex flex-col gap-1">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border border-gray-200 rounded-[8px] overflow-hidden">
              ...
              <tbody>
                ...
                {duties.length === 0 && (
                  <tr>
                    <td colSpan={9} className="text-center py-4">No duties found.</td>
                  </tr>
                )}
              </tbody>
            </table>
