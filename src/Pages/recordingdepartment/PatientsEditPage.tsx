...
import { getPatientByIdApi } from "@/api-integration/queries/patients";
...
export default function PatientsEditPage() {
  ...
  const [category, setCategory] = useState<"civilian" | "personnel">("civilian");
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setMiddlename((p as any).middlename || "");
        const isPersonnel = !!p.veteran;
        setCategory(isPersonnel ? "personnel" : "civilian");
        setServiceNumber(p.serviceNumber || "");
        setRank(p.rank || "");
        setMembershipNumber((p as any).membershipNumber || "");
        setSex(p.sex || "");
        setDateOfBirth(p.dateOfBirth || "");
        setDobDate(p.dateOfBirth ? new Date(p.dateOfBirth) : undefined);
        setAge((p.age ?? "") as any);
        setCountry(p.country || "");
        setStateOfOrigin(p.stateOfOrigin || "");
        setLga(p.lga || "");
        ...
        const nk = (p.nok || {}) as any;
        setNokName(nk.name || "");
        setNokRelationship(nk.relationship || "");
        setNokPhone(nk.phone || "");
        setNokAddress(nk.address || "");
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err ?? "");
        toast.error(msg || "Failed to load patient");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const statesList = useMemo(() => (COUNTRIES[country] || []).map((s) => s.state), [country]);
  ...
  return (
    <div className="py-4 w-full">
      <Card className="w-full mx-auto">
        <CardHeader>
          <CardTitle>Edit Patient Biodata</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex flex-col gap-1">
              ...
              <Select value={country} onValueChange={(v) => { setCountry(v); setStateOfOrigin(""); setLga(""); }}>
                ...
                <SelectContent><SelectGroup>
                  {Object.keys(COUNTRIES).map((c) => (<SelectItem key={c} value={c}>{c}</SelectItem>))}
                </SelectGroup></SelectContent>
              </Select>
            </div>
            <div className="flex flex-col gap-1">
          ...
          <div className="flex gap-2">
            ...
            <Button className="bg-[#56bbe3] text-white hover:bg-[#56bbe3]/80" type="button" onClick={onSubmit} disabled={update.isPending}>
