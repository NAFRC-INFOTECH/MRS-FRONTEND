...
import { Beaker, CalendarClock, ClipboardList, FlaskConical, TestTube2 } from "lucide-react";
...
import { useLabReferralsByPatientQuery, type LabReferral } from "@/api-integration/queries/lab";
...
export default function LabResultsTable({ patientId }: { patientId: string }) {
  ...
  const q = useLabReferralsByPatientQuery(patientId, statsParams);

  const rows = useMemo(() => {
    const list = (q.data || []) as LabReferral[];
  ...
  const statusBadge = (status: LabReferral["status"]) => {
  ...
  const renderDetails = (r: LabReferral) => {
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div className="rounded-lg border p-3">
            <div className="text-xs text-muted-foreground">Specimen</div>
            <div className="font-medium">{r.specimen || "-"}</div>
          </div>
          <div className="rounded-lg border p-3">
          ...
          <div className="rounded-md border p-3 md:col-span-2">
            ...
            <div className="mt-1 flex flex-wrap gap-1">
              ...
              {tests.length > 8 && (
                <Badge variant="outline">+{tests.length - 8} more</Badge>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border overflow-hidden">
          <div className="flex items-center justify-between bg-gradient-to-r from-[#56bbe3] to-cyan-500 px-4 py-2 text-white">
          ...
          <div className="overflow-x-auto bg-white border border-gray-200 rounded-md">
            <table className="min-w-full border-collapse">
              <thead className="bg-[#f2fbff]">
                <tr>
                  <th className="pl-4 py-2 text-left text-md font-semibold text-gray-700 whitespace-nowrap border-r border-gray-200">
                    S/N
                  </th>
                  <th className="px-4 py-2 text-left text-md font-semibold text-gray-700 border-r border-gray-200">
                    Test
                  </th>
                  ...
                  return (
                    <tr
                      ...
                      className={cn(
                        ...
                        idx % 2 === 1 ? "bg-[#fbfbfb]" : "bg-white"
                      )}
                    >
                      <td className="px-4 py-2 text-sm whitespace-nowrap border-r border-gray-200">
                        {idx + 1}
                      </td>
                      ...
                      <td
                        className={cn(
                          "px-4 py-2 text-sm",
                          val ? "text-gray-900" : "text-muted-foreground"
                        )}
                      >
                        {val || "-"}
                      </td>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="text-xs text-muted-foreground">Referrals and lab outcomes for this patient</div>
          </div>
      ...
      {q.isLoading && (
        <div className="rounded-lg border bg-white p-4 text-sm text-muted-foreground">Loading lab results...</div>
      )}
      {q.isError && !q.isLoading && (
        <div className="rounded-lg border bg-white p-4 text-sm text-red-600">Failed to load lab results.</div>
      )}
          ...
          <CardHeader className="space-y-1">
            <CardTitle className="flex items-center justify-between gap-2">
              <span className="flex items-center gap-2">
                <FlaskConical className="h-5 w-5 text-[#56bbe3]" />
                Most Recent Lab Result
              </span>
              {statusBadge(latest.status)}
            </CardTitle>
            <CardDescription>
              {new Date(latest.date).toLocaleString()} • {latest.specimen || "Specimen -"} • {latest.to || "Lab"}
            </CardDescription>
          </CardHeader>
          <CardContent className="flex items-center justify-between gap-3">
            <div className="text-sm text-muted-foreground">
              {normalizeTests(latest.examinationRequired).length} tests requested
            </div>
            <Button onClick={() => openDetails(latest.id)} className="bg-[#56bbe3] text-white hover:bg-[#56bbe3]/80">
              View Details
            </Button>
          </CardContent>
        </Card>
      ...
      {!q.isLoading && !q.isError && rows.length === 0 && (
        <Card className="shadow-none">
          <CardHeader>
            <CardTitle>No lab referrals</CardTitle>
            <CardDescription>No lab records found for the selected period.</CardDescription>
          </CardHeader>
        </Card>
      )}
      ...
      {!q.isLoading && !q.isError && rows.length > 0 && (
        <Card className="shadow-none">
          <CardHeader>
            <CardTitle>Lab Referrals</CardTitle>
            <CardDescription>Click any row to view requested tests and results.</CardDescription>
          </CardHeader>
              ...
              <table className="min-w-full border-t border-gray-200">
                <thead className="bg-[#56bbe3] text-white">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left whitespace-nowrap">Specimen</th>
                  ...
                  {rows.map((r, idx) => {
                    ...
                    return (
                      <tr
                        key={r.id}
                        className={cn(
                          "border-b border-gray-200 even:bg-[#f9f9f9] cursor-pointer hover:bg-[#eefbff]",
                          idx === 0 ? "bg-[#eefbff]" : ""
                        )}
                      ...
                      >
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-sm">{r.specimen || "-"}</td>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="w-full max-w-6xl lg:ml-30 lg:mt-10 lg:min-w-3xl mr-0">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Beaker className="h-5 w-5 text-[#56bbe3]" />
              Lab Result Details
            </DialogTitle>
            <DialogDescription>
